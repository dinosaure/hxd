      ...
      | Some (res, off', len') ->
        ...
        then
          ( (* XXX(dinosaure): deferred inputs. *)
            ...
            ic.contents <- ( fun () -> if !consumed then ic.contents ()
                             else ( consumed := true
...
let lseek =
  { lseek= fun _ pos mode ->
        let res = match pos, mode with
...
let o configuration ic oc ppf =
  ...
  let res = O.o configuration lwt (module Lwt_iflow) (module Lwt_oflow) ic oc lseek (`Absolute 0) ppf in
  let open Lwt.Infix in
  Lwt_scheduler.prj res >>= function
  | Ok () -> oc None
  | Error (_ : error) -> .
