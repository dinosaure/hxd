...
let notzen =
  for i = 0 to 31 do x.(i) <- `Style (`Fg, `Cyan) done ;
  for i = 48 to 57 do x.(i) <- `Style (`Fg, `Green) done ;
  for i = 65 to 90 do x.(i) <- `Style (`Fg, `Magenta) done ;
  for i = 97 to 122 do x.(i) <- `Style (`Fg, `Blue) done ;
...
let do_cmd () cols groupsize long uppercase seek ic oc =
  ...
  let configuration = Hxd.O.configuration ?cols ?groupsize ?long ~uppercase notzen in
  Hxd_unix.o configuration ic oc seek null ;
  ic_close () ; oc_close () ; `Ok ()
