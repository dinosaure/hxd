...
module Option = struct
  let map_value ~default f = function Some x -> f x | None -> default
end
...
let colorscheme_of_array : Fmt.style array -> colorscheme =
 fun x ->
  if Array.length x <> 256
  then Fmt.invalid_arg "Hxd.O.colorscheme_of_array: expect 256 elements" ;
  x
...
let fmt_of_byte uppercase : Format.formatter -> int -> unit =
 fun ppf x ->
...
let pp_like_xxd (xxd : xxd) ~seek ppf chunk =
  ...
  while !off < xxd.cols do
    if !off < len
       && Option.map_value ~default:true
            (fun long -> seek + !off < long)
            xxd.long
    then
      let code = Char.code chunk.[!off] in
      (Fmt.styled xxd.colorscheme.(code) (fmt_of_byte xxd.uppercase)) ppf code
    else Format.pp_print_string ppf "  " ;

    if xxd.groupsize <> 0 && (!off + 1) mod xxd.groupsize = 0
...
let pp_like_caml caml ~seek:_ ppf chunk =
  ...
  for i = 0 to String.length chunk - 1 do
    Format.fprintf ppf
      ("\\x" ^^ if caml.uppercase then "%02X" else "%02x")
      (Char.code chunk.[i])
  done ;
  ...
  then
    for _ = String.length chunk to caml.cols - 1 do
      Format.pp_print_string ppf "    "
    done ;

  if caml.with_comments then Format.fprintf ppf " (* %a *)" pp_chunk chunk ;
...
let pp_begin_of_cfg : cfg -> Format.formatter -> unit -> unit = function
  | Xxd _ -> fun _ppf () -> ()
  | Caml caml ->
  match caml.kind with
  | `List -> fun ppf () -> Format.pp_print_string ppf "[\n"
...
let xxd ?(cols = 16) ?(groupsize = 2) ?long
    ?buffer_size:(i_buffer_size, o_buffer_size = (4096, 4096))
    ?(uppercase = false) colorscheme =
  Xxd
    {
      cols;
      groupsize;
...
let caml ?(with_comments = false) ?(cols = 16) ?long
    ?buffer_size:(i_buffer_size, o_buffer_size = (4096, 4096))
    ?(uppercase = false) kind =
  Caml
 ...
 fun cfg s (module IFlow) (module OFlow) ic oc { lseek } seek ppf ->
  ...
  let rec trailing_output seek =
    ...
    then return (Ok ())
      ...
      Ke.keep_exn quo ~blit:B.blit_to_bytes ~length:Bytes.length ~off:0 ~len
        otmp ;
      OFlow.output oc (Bytes.unsafe_to_string otmp) ~off:0 ~len >>= function
      | Ok wrote ->
          Ke.shift_exn quo wrote ;
          trailing_output seek
  ...
  and flush_output seek =
    match
      Ke.keep qui ~blit:B.blit_to_bytes ~length:Bytes.length ~off:0
        ~len:(cols cfg) cur
    with
    ...
    | Some () -> (
        ...
        | None -> (
            let len = (min : int -> int -> int) (Ke.length quo) o_buffer_size in
            Ke.keep_exn quo ~blit:B.blit_to_bytes ~length:Bytes.length ~off:0
              ~len otmp ;
            OFlow.output oc (Bytes.unsafe_to_string otmp) ~off:0 ~len
            >>= function
            | Ok wrote ->
                Ke.shift_exn quo wrote ;
                flush_input seek
  ...
  and flush_input seek =
    if Ke.available qui = 0
    then flush_output seek
    else
      let len = (min : int -> int -> int) i_buffer_size (Ke.available qui) in
      input ic itmp ~off:0 ~len >>= function
      | Error _ as err -> return err
      | Ok 0 -> trailing_input seek
      | Ok len ->
          let _ =
            Ke.push_exn qui ~blit:B.blit_of_bytes ~length:Bytes.length ~off:0
              ~len itmp in
  ...
  | `Absolute position ->
      if position < 0
      then
        lseek ic position `END >>= function
        | Ok _ -> flush_input 0
        | Error _ as err -> return err
